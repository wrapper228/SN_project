...
Для преподавателя в репозитории приложена история реального Telegram-диалога с ботом:

- задача: *"у меня на рабочем столе есть чеки из вышки. изучи их и расскажи про них"*
- управление потоком выполнения задачи: *"еще спроси потом, что мне делать с этими чеками чтобы получить нал вычет"*
- основной файл: [bot_demo.pdf](bot_demo.pdf)
- исходный экспорт: [ChatExport_2026-05-13](ChatExport_2026-05-13)
- краткое описание: [docs/chat-history.md](docs/chat-history.md)

PDF добавлен специально для удобного просмотра прямо из репозитория. В нём видно:

- успешный `/start`;
- создание задач через Telegram;
- получение ботом и backend реальных task id;
- промежуточные сообщения и скриншоты;
- interrupt текущей задачи новым сообщением;
- итоговый ответ по выполненной задаче.
